...
Unfortunately it doesn't have flashy visuals like the DirectX project does (yet). Might be a wee bit- according to "Game Engine Architecture" by Jason Gregory (Uncharted and The Last Of Us), having every game object inheret from an interface with draw(), input(), and update() is wrong (or just not right). He then goes into the two common ways games are set up, one of which I am now trying to emulate.It's a fairly large step away from what I was previously doing. 

Also I need to get compiling on mac. And hopefully linux via VirtualBox.
