...
//Default BRDF
float3 BRDF( float3 lightDir, float3 viewDir, float NdotV, float3 lightDiffuse, float3 lightSpecular, Material material, float3 nNormal , float3 diffuseCol )
{
...
float4 diffuseCol;
	...
	float3 finalColour = float3(0, 0, 0);
		...
		finalColour += BRDF( passBuf.lights[0].position.xyz, viewDir, NdotV, passBuf.lights[0].diffuse, passBuf.lights[0].specular, material, nNormal , diffuseCol.xyz ) * fShadow;
	...
	if( fDistance <= passBuf.lights[1].attenuation.x && spotCosAngle >= passBuf.lights[1].spotParams.y  )
	{
		...
		tmpColour = BRDF( lightDir, viewDir, NdotV, passBuf.lights[1].diffuse, passBuf.lights[1].specular, material, nNormal , diffuseCol.xyz )
	...
	if( fDistance <= passBuf.lights[2].attenuation.x && spotCosAngle >= passBuf.lights[2].spotParams.y  )
	{
		...
		tmpColour = BRDF( lightDir, viewDir, NdotV, passBuf.lights[2].diffuse, passBuf.lights[2].specular, material, nNormal , diffuseCol.xyz )
	...
	for( uint i=0u; i<numLightsInGrid; ++i )
	{
		...
		[branch]if( fDistance <= attenuation.x  )
		{
			...
			//Point light
			float3 tmpColour = BRDF( lightDir, viewDir, NdotV, lightDiffuse.xyz, lightSpecular, material, nNormal , diffuseCol.xyz );
