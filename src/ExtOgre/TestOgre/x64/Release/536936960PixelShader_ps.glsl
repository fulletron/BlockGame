...
vec3 BRDF( vec3 lightDir, vec3 viewDir, float NdotV, vec3 lightDiffuse, vec3 lightSpecular )
{
	...
	#define OGRE_SAMPLE_SHADOW( tex, sampler, uv, depth ) (depth >= 1.0 ? 1.0 : texture( tex, vec3( uv, depth ) ))
...
void main()
{
	...
	/// Sample detail maps and weight them against the weight map in the next foreach loop.
		...
		diffuseCol = texture( textureMaps[0],
														vec3( UV_DIFFUSE( inPs.uv0.xy ),
															  diffuseIdx ) );
	...
	/// Blend the detail diffuse maps with the main diffuse.
		...
		nNormal = getTSNormal( vec3( UV_NORMAL( inPs.uv0.xy ),
																normalIdx ) );
	...
	/// initialize nNormal instead of try to merge with it.
		...
		if( inPs.depth <= passBuf.pssmSplitPoints0 )
		{
			fShadow = getShadow( hlms_shadowmap0, 
				...
				{
					fShadowBlend = getShadow( hlms_shadowmap1, 
											  inPs.posL1,
		...
		else if( inPs.depth <= passBuf.pssmSplitPoints1 )
		{
			fShadow = getShadow( hlms_shadowmap1, 
				...
				{
					fShadowBlend = getShadow( hlms_shadowmap2, 
											  inPs.posL2,
		...
		ROUGHNESS = material.kS.w * texture( textureMaps[2],
											vec3( UV_ROUGHNESS( inPs.uv0.xy ),
												  roughnessIdx) ).x;
	...
	specularCol = texture( textureMaps[0],
														vec3( UV_SPECULAR( inPs.uv0.xy ),
	...
	if( fDistance <= passBuf.lights[1].attenuation.x && spotCosAngle >= passBuf.lights[1].spotParams.y  )
	{
		...
		float spotAtten = clamp( (spotCosAngle - passBuf.lights[1].spotParams.y) * passBuf.lights[1].spotParams.x, 0.0, 1.0 );
		spotAtten = pow( spotAtten, passBuf.lights[1].spotParams.z );
	
		tmpColour = BRDF( lightDir, viewDir, NdotV, passBuf.lights[1].diffuse, passBuf.lights[1].specular )
		* getShadow( hlms_shadowmap3, 
					 inPs.posL3,
	...
	if( fDistance <= passBuf.lights[2].attenuation.x && spotCosAngle >= passBuf.lights[2].spotParams.y  )
	{
		...
		float spotAtten = clamp( (spotCosAngle - passBuf.lights[2].spotParams.y) * passBuf.lights[2].spotParams.x, 0.0, 1.0 );
		spotAtten = pow( spotAtten, passBuf.lights[2].spotParams.z );
	
		tmpColour = BRDF( lightDir, viewDir, NdotV, passBuf.lights[2].diffuse, passBuf.lights[2].specular )
		* getShadow( hlms_shadowmap4, 
					 inPs.posL4,
