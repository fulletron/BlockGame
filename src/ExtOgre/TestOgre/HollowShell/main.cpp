...
#include "othreading/yieldtimer.h"
...
const double cFrametime = 1.0 / 240.0;
...
#endif
{
	OGraphics::F3DMultiGraphicsGameState graphicsGameState(
		...
		"weight Z, etc)");
	F3DMultiGraphicsSystem graphicsSystem(&graphicsGameState);
	F3DMultiLogicGameState logicGameState;
	...
	// KYLE :: NOTE :: This makes both point to eachother. Not particularly good?
	logicSystem._notifyLogicGameState(&logicGameState);
...
unsigned long renderThreadApp(Ogre::ThreadHandle *threadHandle)
{
	...
	graphicsSystem->initialize("Tutorial 06: Multithreading", threadData->logicSystem);
	...
	double timeSinceLast = 1.0 / 240.0;
