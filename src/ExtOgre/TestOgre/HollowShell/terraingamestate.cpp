...
#include "oterra/terra.h"
#include "oterra/terrashadowmapper.h"
#include "oterra/Hlms/PbsListener/OgreHlmsPbsTerraShadows.h"
...
//#define KYLE_EMPTY_PROJECT
#define KYLE_INC_TERRA 1
...
namespace OGraphics
{
	...
	Ogre::CompositorWorkspace* Tutorial_TerrainGameState::setupCompositor()
	{
		...
		Root *root = mpGraphicsSystem->getRoot();
		SceneManager *sceneManager = mpGraphicsSystem->getSceneManager();
		RenderWindow *renderWindow = mpGraphicsSystem->getRenderWindow();
		Camera *camera = mpGraphicsSystem->getCamera();
		CompositorManager2 *compositorManager = root->getCompositorManager2();

		CompositorWorkspace *oldWorkspace = mpGraphicsSystem->getCompositorWorkspace();
		if (oldWorkspace)
		{
...
//		mTerra->load( "Heightmap.png", Ogre::Vector3( 64.0f, 4096.0f * 0.5f, 64.0f ), Ogre::Vector3( 14096.0f, 14096.0f, 14096.0f ) );
		...
		mpGraphicsSystem->getCamera()->setFarClipDistance(100000.0f);
		mpGraphicsSystem->getCamera()->setPosition(-10.0f, 80.0f, 10.0f);
	...
	void Tutorial_TerrainGameState::update(float timeSinceLast)
	{
		...
		Ogre::Camera *camera = mpGraphicsSystem->getCamera();
	...
	void Tutorial_TerrainGameState::generateDebugText(float timeSinceLast, Ogre::String &outText)
	{
		...
		else if (mDisplayHelpMode == 1)
		{
			...
			Ogre::Vector3 camPos = mpGraphicsSystem->getCamera()->getPosition();
