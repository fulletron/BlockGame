...
namespace OGraphics
{
	...
	{
		Ogre::SceneManager *sceneManager = mpGraphicsSystem->getSceneManager();
...
//		sceneManager->setForwardClustered(true, 16, 8, 24, 96, 5, 500);
...
#ifdef USE_RQGROUP_VISFLAGS 1
			item->setRenderQueueGroup(1);
			item->setVisibilityFlags(D_VISIBILITYFLAG_1);
#endif
#ifdef USE_BASEWORLDNODE 1
			sceneNode = mpGraphicsSystem->getSceneNodeWorld()->createChildSceneNode(Ogre::SCENE_DYNAMIC);
#else
...
#endif
#ifdef USE_BASEWORLDNODE 1
				mSceneNode[idx] = mpGraphicsSystem->getSceneNodeWorld()->
					createChildSceneNode(Ogre::SCENE_DYNAMIC);
#else
...
#endif
#ifdef USE_BASEWORLDNODE 1
			Ogre::SceneNode *sceneNode = mpGraphicsSystem->getSceneNodeWorld()->
...
#ifdef USE_BASEWORLDNODE 1
		Ogre::SceneNode *rootNode = mpGraphicsSystem->getSceneNodeWorld();
#else
...
#ifdef USE_RQGROUP_VISFLAGS 1
		light->setRenderQueueGroup(1);
		light->setVisibilityFlags(D_VISIBILITYFLAG_1);
#endif
		// DIRECTIONAL LIGHT WILL BECOME SHITTIER FASTER BECAUSE IT ALL GETS MAPPED AT ONCE
		// SPOT LIGHTS COVER A SMALLER AREA, AND THUS HIGHER RESOLUTION/GRANULARITY
		...
		lightNode = rootNode->createChildSceneNode();
		lightNode->attachObject(light);
		light->setDiffuseColour(1.0f, 0.0f, 1.0f); //Cold
		light->setSpecularColour(0.2f, 0.4f, 0.8f);
		light->setPowerScale(Ogre::Math::PI);
		light->setType(Ogre::Light::LT_SPOTLIGHT);
		//light->setType(Ogre::Light::LT_POINT);
		lightNode->setPosition(0.0f, 0.2f, 1.0f); // mpCameraUI->setPosition(Ogre::Vector3(0, 0, 1));
		light->setDirection(Ogre::Vector3(0, 0, 0).normalisedCopy()); // mpCameraUI->lookAt(Ogre::Vector3(0, 0, 0));
		light->setAttenuationBasedOnRadius(10.0f, 0.01f);
#ifdef USE_RQGROUP_VISFLAGS 1
		light->setRenderQueueGroup(1);
		light->setVisibilityFlags(D_VISIBILITYFLAG_1);
#endif
		mLightNode4 = lightNode;
#endif
		...
		//pbs->setEsmK(1200);
/*
		Ogre::Hlms *hlms = mpGraphicsSystem->getRoot()->getHlmsManager()->getHlms(Ogre::HLMS_UNLIT);
		assert(dynamic_cast<Ogre::HlmsUnlit*>(hlms));
		Ogre::HlmsUnlit *unlit = static_cast<Ogre::HlmsUnlit*>(hlms);
*/
	...
	void Forward3DGameState::generateUIQuad(void)
	{
#ifdef USE_BASEWORLDNODE 1
		Ogre::SceneManager *sceneManager = mpGraphicsSystem->getSceneManager();
		Ogre::Item *item = sceneManager->createItem("Cube_d.mesh", //"cube_testkyle.mesh",
			Ogre::ResourceGroupManager::AUTODETECT_RESOURCE_GROUP_NAME,
			Ogre::SCENE_DYNAMIC);
...
#ifdef USE_RQGROUP_VISFLAGS 1
		item->setRenderQueueGroup(2);
		item->setVisibilityFlags(D_VISIBILITYFLAG_2);
#endif

		mUIQUAD = mpGraphicsSystem->getSceneNodeUI()->
			createChildSceneNode(Ogre::SCENE_DYNAMIC);

		mUIQUAD->setPosition(0,0,0);
		mUIQUAD->setScale(0.1f, 0.1f, 0.1f);

		mUIQUAD->attachObject(item);
#endif
	}
	...
	void Forward3DGameState::generateLights(void)
	{
		...
		for (Ogre::uint32 i = 0; i<mNumLights; ++i)
		{
			...
			light->setPowerScale(Ogre::Math::PI);
			light->setCastShadows(false);// KYLE :: THIS IS A TEST
			light->setType(Ogre::Light::LT_POINT);
	...
	void Forward3DGameState::update(float timeSinceLast)
	{
		if (mAnimateObjects)
		{
			...
			mUIQUAD->yaw(Ogre::Radian(timeSinceLast * 0.125f));
		}
