	...
	, mTextSize(1.0f)
{
    if (params != 0)
    {
		...
		ni = params->find("isBillboard");
		if (ni != params->end())
		{
//			if (ni->second != Ogre::String("false"))
//				mIsBillboard = true;
			mIsBillboard = Ogre::StringConverter::parseBool(ni->second);
		}

		ni = params->find("textSize");
		if (ni != params->end())
		{
			mTextSize = Ogre::StringConverter::parseReal(ni->second);
		}
    }
...
void MovableText::setFontName(const String &fontName)
{
	...
	if (mFontName != fontName || mpHlmsDatablock == NULL || !mpFont)
	{
		...
		reinterpret_cast<HlmsUnlitDatablock*>(mpHlmsDatablock)->setTextureSwizzle(
			0, 
			HlmsUnlitDatablock::R_MASK, 
			HlmsUnlitDatablock::R_MASK,
			HlmsUnlitDatablock::R_MASK,
			HlmsUnlitDatablock::G_MASK
			);
...
void MovableText::_setupGeometry()
{
    ...
    if (!mRenderOp.vertexData)
        mRenderOp.vertexData = new v1::VertexData();
 
    mRenderOp.indexData = 0;
    mRenderOp.vertexData->vertexStart = 0;
    mRenderOp.vertexData->vertexCount = vertexCount;
//    mRenderOp.operationType = v1::RenderOperation::OT_TRIANGLE_LIST;
...
{
    if (this->isVisible() && mpCam && mIsBillboard)
    {
...
bool MovableText::frameEnded(const FrameEvent &evt)
{
	return true;
}
