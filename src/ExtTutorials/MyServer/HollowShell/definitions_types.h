...
//#define DBGCOLOR(x) SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), x );
//#define DBGCOLORRESET DBGCOLOR(FOREGROUND_RED | FOREGROUND_BLUE | FOREGROUND_GREEN );

//#define DEBUG_OUT(x) std::cout << x << std::endl;
//#define DEBUG_OUT_RED(x) DBGCOLOR(FOREGROUND_RED); DEBUG_OUT(x); DBGCOLORRESET; 
//#define DEBUG_OUT_YELLOW(x) DBGCOLOR(FOREGROUND_RED | FOREGROUND_GREEN); DEBUG_OUT(x); DBGCOLORRESET;
//#define DEBUG_OUT_GREEN(x) DBGCOLOR(FOREGROUND_GREEN); DEBUG_OUT(x); DBGCOLORRESET;

#define DEBUG_OUT(x) Logger::get().log( (FOREGROUND_RED | FOREGROUND_BLUE | FOREGROUND_GREEN), x );
//#define DEBUG_OUT_T(x) Logger::get().log_t( x );
#define DEBUG_OUT_RED(x) Logger::get().log( (FOREGROUND_RED), x );
#define DEBUG_OUT_YELLOW(x) Logger::get().log( (FOREGROUND_RED | FOREGROUND_GREEN), x);
#define DEBUG_OUT_GREEN(x) Logger::get().log( (FOREGROUND_GREEN), x);

#else
#define DEBUG_OUT(x) ;
#define DEBUG_OUT_RED(x) ;
#endif
#else
#define DEBUG_OUT(x) ;
#define DEBUG_OUT_RED(x) ;
